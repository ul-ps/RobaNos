/* ========================================================
   game.js — Individual Game Page Logic | GameVault
======================================================== */
...
window.addEventListener('firebaseDataReady', () => {
  // Merge approved custom games
  try {
    const extraGames = JSON.parse(localStorage.getItem('gv_extra_games') || '[]');
    if (extraGames.length > 0) {
      // Filter out standalone videos (type field)
      const filtered = extraGames.filter(g => !g.type);
      GAMES_DATA.push(...filtered);
    }
  } catch (e) {
    console.error(e);
  }
  ...
  // Handle both integer IDs and string IDs (EXT_...)
  let game = GAMES_DATA.find(g => g.id == gameId);
  let isPreview = false;

  // Fallback for previewing pending submissions
  if (!game) {
    const subs = JSON.parse(localStorage.getItem('gv_submissions') || '[]');
    game = subs.find(s => s.id == gameId || s.subId == gameId);
    if (game) isPreview = true;
  }
...
function getEmbedUrl(url) {
  ...
  if (match && match[1]) return `https://www.youtube.com/embed/${match[1]}?autoplay=0&rel=0`;
      ...
      <div>
        ...
        <div class="gp-section">
          ...
          <p class="gp-desc" style="font-size:0.9rem">
            كل الملفات المعروضة في المنصة خضعت للمراجعة من قبل مشرفي <strong>GameVault</strong> للتأكد من خلوها من البرمجيات الخبيثة.
          </p>
        </div>
...
function renderComments(gameId) {
    ...
    if (currUser) {
      ...
      inputEl.innerHTML = `
        ...
        <style>
          ...
          .comment-avatar-circle { width:48px; height:48px; border-radius:50%; background:linear-gradient(135deg,var(--primary),var(--accent)); display:flex; align-items:center; justify-content:center; font-size:1.3rem; font-weight:bold; color:white; flex-shrink:0; box-shadow:0 5px 15px rgba(168,85,247,0.3); }
          #comment-input { width:100%; min-height:80px; background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.1); border-radius:14px; padding:15px; color:white; font-family:inherit; font-size:1rem; resize:vertical; outline:none; transition:all 0.3s; box-shadow:inset 0 4px 10px rgba(0,0,0,0.1); display:block; }
      ...
      inputEl.innerHTML = `
        <div style="background:rgba(255,255,255,0.02); border:1px dashed rgba(255,255,255,0.15); border-radius:16px; padding:35px 20px; text-align:center; margin-bottom:40px;">
           <div style="font-size:2.5rem; margin-bottom:12px; opacity:0.8">💬</div>
           <h4 style="color:#fff; margin-bottom:8px; font-size:1.2rem; font-weight:700;">شارك في حوار المجتمع</h4>
  ...
  list.innerHTML = sorted.map((c, i) => `
    <div style="display:flex; gap:16px; padding: 25px 15px; border-bottom: ${i === sorted.length - 1 ? 'none' : '1px solid rgba(255,255,255,0.06)'}; transition: background 0.3s; border-radius: 12px;" onmouseover="this.style.background='rgba(255,255,255,0.02)'" onmouseout="this.style.background='transparent'">
      <div style="width:48px; height:48px; border-radius:50%; background:linear-gradient(135deg,var(--primary),var(--accent)); display:flex; align-items:center; justify-content:center; color:white; font-weight:bold; font-size:1.3rem; flex-shrink:0;">
        <i class="fas fa-user-astronaut"></i>
      </div>
      <div style="flex:1;">
        <div style="display:flex; align-items:center; gap:12px; margin-bottom:8px;">
          <strong style="color:white; font-size:1.1rem; cursor:pointer; font-weight:700" onclick="window.location.href='user.html?user=${encodeURIComponent(c.user)}'" onmouseover="this.style.color='var(--primary-light)'" onmouseout="this.style.color='white'">${c.user}</strong>
...
function shareGame(name) {
  ...
  if (navigator.share) {
    navigator.share({
      title: name + ' - GameVault',
      text: 'شاهد هذه اللعبة الرائعة على GameVault!',
      url: url
...
function reportGame() {
  ...
  // Create Report Modal Dynamically
  let modal = document.getElementById('report-modal');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'report-modal';
    modal.style.cssText = 'position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.85); z-index:1000000; display:flex; align-items:center; justify-content:center; backdrop-filter:blur(8px); opacity:0; transition:opacity 0.3s;';
    modal.innerHTML = `
      <div class="report-modal-box">
        <button onclick="closeReportModal()" class="report-close-btn"><i class="fas fa-times"></i></button>
        <h3 class="report-modal-title">
          <i class="fas fa-flag" style="color:#f87171"></i> الإبلاغ عن مشكلة
        </h3>
        <p class="report-modal-desc">نشكرك على اهتمامك بسلامة المحتوى. يرجى توضيح سبب الإبلاغ عن لعبة <strong>${game.name}</strong> ليقوم فريق الإشراف بمراجعتها.</p>
        
        <div class="report-field">
          <label class="report-label">السبب:</label>
          <select id="report-reason" class="report-select">
            <option value="link_broken">الروابط لا تعمل</option>
            <option value="wrong_content">محتوى غير لائق أو مخالف</option>
            <option value="malware">ملف ضار أو فيروسات</option>
        ...
        <div class="report-field">
          <label class="report-label">تفاصيل إضافية (اختياري):</label>
          <textarea id="report-details" class="report-textarea" placeholder="اكتب هنا أي تفاصيل قد تساعدنا في حل المشكلة..."></textarea>
        </div>
        
        <button class="report-submit-btn" onclick="submitGameReport('${gameId}', '${game.name.replace(/'/g, "\\'")}')">
          إرسال البلاغ الآن
        </button>
      </div>
    `;
...
window.submitGameReport = function(gameId, gameName) {
  ...
  // Get current user for better reporting and notifications
  ...
  // Send notification to the user if logged in
  if (user && typeof addNotification === 'function') {
    addNotification(
      user.name, 
      'info', 
      '✅ تم استلام بلاغك', 
      `شكراً لك! تلقينا بلاغك بخصوص "${gameName}". فريق الإشراف سيقوم بمراجعته وموافقتك بالرد في أقرب وقت.`
    );
  }
  
  closeReportModal();
  
  // Use custom toast if available, otherwise fallback to alert
  if (window.showToast) {
    window.showToast('✅ شكراً لك! تم إرسال البلاغ بنجاح وسيقوم فريق الإشراف بمراجعته.');
  } else {
