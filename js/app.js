/* ========================================================
   app.js — Main Site Logic | GameVault (v2)
======================================================== */
...
function renderHeroShowcase() {
  ...
  if (metaSpans.length >= 2) {
    metaSpans[0].innerHTML = `<i class="fas fa-star" style="color:#fbbf24"></i> ${topGame.rating || '4.5'}`;
    ...
    return `
      <div class="featured-card rank-${rank}" onclick="window.location.href='game.html?id=${g.id}'">
        <div class="featured-card-bg">
          <div class="featured-card-bg-emoji" style="${g.imageUrl ? `background:url('${g.imageUrl}') center/cover no-repeat;color:transparent;` : `background:linear-gradient(135deg,${colors})`}">${getCategoryIcon(g.category, g.emoji)}</div>
          <div class="featured-card-shine"></div>
        </div>
        <div class="featured-huge-rank">${rank}</div>
        <div class="featured-card-overlay"></div>
        <div class="featured-card-content">
          ${(() => {
            const b = getGameBadge(g, GAMES_DATA);
            return b ? `<div style="margin-bottom:6px"><span class="game-badge badge-${b}">${badgeLabel(b)}</span></div>` : '';
          })()}
          <div class="game-category">${g.category}</div>
          <div class="game-name" style="font-size: 1.25rem; margin-bottom:8px">${g.name}</div>
          ${g.submittedBy ? `
          <div class="featured-submitter">
            <div class="featured-submitter-avatar"><i class="fas fa-user-astronaut"></i></div>
            <span style="font-size:0.75rem;color:var(--text-muted)">رُفعت بواسطة <strong style="color:var(--text-main)">${g.submittedBy}</strong></span>
          </div>` : ''}
          <div style="display:flex;align-items:center;gap:14px;font-size:0.82rem;color:var(--text-muted);">
            <span style="color:#fbbf24">⭐ ${g.rating}</span>
            <span>📥 ${formatNumber(g.downloads)}</span>
          </div>
        </div>
      </div>`;
  }).join('');
...
function renderGames(games, page) {
  ...
  grid.innerHTML = pageGames.map((g, i) => {
    const colors = CATEGORY_COLORS[g.category] || 'hsl(260,50%,12%), hsl(280,40%,9%)';
    const stars = renderStars(g.rating);
    return `
      <div class="game-card" style="animation-delay:${i * 0.05}s" onclick="window.location.href='game.html?id=${g.id}'">
        ...
        <div class="game-info">
          ...
          <div class="game-stars">${stars}<span>${g.rating}</span></div>
          ${g.submittedBy ? `
          <div style="display:flex;align-items:center;gap:6px;margin:8px 0;padding:4px 8px;background:rgba(255,255,255,0.03);border-radius:12px;width:fit-content">
            <div style="width:20px;height:20px;border-radius:50%;background:linear-gradient(135deg,var(--primary),var(--accent));display:flex;align-items:center;justify-content:center;color:white;font-weight:bold;font-size:0.6rem"><i class="fas fa-user-astronaut"></i></div>
            <span style="font-size:0.7rem;color:var(--text-muted)">رُفعت بواسطة <strong style="color:var(--text-main)">${g.submittedBy}</strong></span>
          </div>` : ''}
          <div class="game-meta">
        ...
        <div class="game-footer">
          ...
          <button class="btn-download" onclick="event.stopPropagation();openModal(${g.id})">
...
function openModal(id) {
  ...
  document.getElementById('modal-rating').innerHTML = `<span style="color:#fbbf24">⭐</span> ${g.rating}/5`;
  ...
  if (starsEl) starsEl.innerHTML = renderStars(g.rating) + `<span style="margin-right:6px;color:var(--text-muted);font-size:0.88rem">(${g.rating}/5)</span>`;
...
function handleDownload(e) {
  e.preventDefault();
  closeModal();
  showToast('🎮 جاري التحميل... شكراً لاختيارك GameVault!', 'success');
}
