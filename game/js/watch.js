...
function findGame(id) {
    ...
    try {
        ...
        allSources = [...core, ...extraGames, ...extraVideos, ...subs];
    } catch(e) {
        console.error("Data merge error in watch.js:", e);
    }

    // Try to find by ID (checking both string and number) or originalSubId (for published games)
    return allSources.find(g => g.id == id || g.subId == id || g.originalSubId == id);
}
...
function renderVideo(game) {
    ...
    const authorName = game.submittedBy || 'GameVault';
