console.log('🚀 RobaNos Dashboard JS [v2.1-ARMOURED] Initializing...');
...
/* ========================================================
   dashboard.js — Admin Dashboard Logic | RobaNos
 ======================================================== */
...
function showPage(name, el) {
  ...
  const titles = {
    ...
    push: 'مركز الإشعارات (Push Center)'
  };
...
function renderRecentTable() {
  ...
  tbody.innerHTML = recent.map(g => {
    ...
    <tr>
      ...
      <td><span style="color:#fbbf24"><i class="fas fa-star"></i> ${getGameRating(g)}</span></td>
    ...
    <tr>
      ...
      <td>
        <div style="display:flex;gap:6px">
          <button class="action-btn" onclick="editGame('${g.id}')" title="تعديل"><i class="fas fa-pen"></i></button>
          <button class="action-btn del" onclick="deleteGame('${g.id}')" title="حذف"><i class="fas fa-trash-alt"></i></button>
        </div>
      </td>
    </tr>
  `}).join('');
  ...
  if (topRatedEl) {
    ...
    topRatedEl.innerHTML = topRated.map((g, i) => `
      <div class="top-game-item">
        ...
        <div style="color:#fbbf24;font-weight:700"><i class="fas fa-star"></i> ${getGameRating(g)}</div>
      </div>
    `).join('');
  }
}
...
function approveReview(subId) {
  ...
  if (typeof addNotification === 'function') {
    ...
    addNotification(notifyUser, 'success', title, msg);
    
    // Trigger Real-time Push
    if (window.triggerTargetedPush) {
      window.triggerTargetedPush(notifyUser, title, msg);
    }
  }
    ...
    onConfirm: () => {
      ...
      if (idx !== -1) {
        ...
        if (typeof addNotification === 'function') {
          ...
          const msg = `عذراً، تم رفض طلبك بخصوص لعبة "${sub.name}". يرجى مراجعة معايير النشر.`;
          
          addNotification(notifyUser, 'error', title, msg);
...
function editGame(id) {
  ...
  document.getElementById('fg-video').value = g.videoUrl || '';
...
function saveGame() {
  ...
  const videoUrl = document.getElementById('fg-video') ? document.getElementById('fg-video').value.trim() : '';
  ...
  const gameObj = {
    name, category, desc, size, rating, status, badge: '', emoji, imageUrl, videoUrl, screenshots, platforms, links,
...
function saveSettings() {
  ...
  try {
    ...
    // Small timeout to give user sense of "processing"
    setTimeout(() => {
      if (btn) {
        btn.disabled = false;
      ...
      // Apply branding & global changes immediately
      ...
      // Apply maintenance mode console warning for admins
      if (globalSettings.maintenanceMode) {
        console.warn('⚠️ Maintenance Mode is ON (Admin Bypass Active)');
      }
    }, 600);
...
function injectCustomCategoriesInForm() {
  const catSelect = document.getElementById('fg-category');
  if (catSelect) {
    const customCats = JSON.parse(localStorage.getItem('gv_custom_categories') || '[]');
...
// ===== FORM =====
function clearForm() {
  injectCustomCategoriesInForm();
  ['fg-name', 'fg-desc', 'fg-size', 'fg-rating', 'fg-image', 'fg-emoji', 'fg-video'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });
...
async function renderPushPage() {
  ...
  try {
    ...
    // Update sidebar badge if any
    const badge = document.getElementById('nav-badge-push');
    if (badge) badge.style.display = 'none'; // Clear "New" badge once seen
  } catch (error) {
    console.error('Error fetching subscribers:', error);
  }
}

window.broadcastPushNotification = async function() {
  const title = document.getElementById('push-title').value.trim();
  const body = document.getElementById('push-body').value.trim();
  const url = document.getElementById('push-url').value.trim(); // No default fallback

  if (!title || !body) {
    showToast('⚠️ يرجى إدخال عنوان ونص الإشعار', 'error');
    return;
  }

  showCustomConfirm({
    title: 'تأكيد البث المباشر',
    message: `هل أنت متأكد من إرسال هذا الإشعار لجميع المشتركين؟ سيتم إرساله لهواتفهم ومتصفحاتهم فوراً.`,
    type: 'info',
    icon: 'fa-paper-plane',
    confirmText: 'إرسال الآن',
    onConfirm: async () => {
      try {
        // Save to broadcast_notifications collection
        // In a real Firebase setup, a Cloud Function would trigger on this and call FCM.
        ...
        // Update history (simulated for simplicity)
        const historyEl = document.getElementById('push-history-short');
        if (historyEl) {
           historyEl.innerHTML = `<div style="padding:10px; border-radius:8px; background:rgba(255,255,255,0.05); margin-top:5px;">
              <div style="font-weight:700; color:var(--primary-light)">${title}</div>
              <div style="font-size:0.75rem">${new Date().toLocaleString('ar-EG')}</div>
           </div>`;
        }
      } catch (error) {
...
/**
 * Helper to trigger a targeted push notification for a specific user
 */
window.triggerTargetedPush = function(userName, title, body) {
  if (!window.db || !userName) return;
  
  db.collection('targeted_push').add({
    targetUser: userName,
    title: title,
    body: body,
    timestamp: firebase.firestore.FieldValue.serverTimestamp(),
    ...
    console.log(`Targeted push triggered for ${userName}`);
