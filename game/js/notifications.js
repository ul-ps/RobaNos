...
function saveTokenToFirestore(token) {
    ...
    db.collection('fcm_subscribers').doc(token).set({
        ...
        // Notify user of success only on dashboard page
...
window.addEventListener('load', () => {
    ...
    // Sync Released Upcoming Games
    setTimeout(syncUpcomingReleaseNotifications, 7000);
});

/**
 * Automatically check if subscribed upcoming games have been released
 * and add a notification to the user's profile.
 */
function syncUpcomingReleaseNotifications() {
    const userStr = localStorage.getItem('gv_session');
    if (!userStr) return;
    ...
    // 2. Get Released Games
    const releasedGames = JSON.parse(localStorage.getItem('gv_extra_games') || '[]');
    
    // 3. Get Existing Notifications
    let allNotifs = JSON.parse(localStorage.getItem('gv_notifications') || '{}');
    let userNotifs = allNotifs[userName] || [];

    let changed = false;

    notifiedIds.forEach(id => {
        // Check if game is now released
        const game = releasedGames.find(g => g.id == id);
        if (game) {
            // Check if we already notified for this specific release
            const alreadyNotified = userNotifs.some(n => n.type === 'game_release' && n.gameId == id);
            ...
            if (!alreadyNotified) {
                ...
                const newNotif = {
                    ...
                    type: 'game_release',
                    gameId: id,
                    title: `🎮 صدرت الآن: ${game.name}`,
                    message: `اللعبة التي كنت تنتظرها "${game.name}" أصبحت متوفرة الآن للتحميل! استمتع باللعب.`,
                    date: new Date().toLocaleString('ar-EG', { dateStyle: 'short', timeStyle: 'short' }),
                    read: false,
                    icon: 'fa-rocket',
                    color: '#10b981', // Success green
                    link: `game.html?id=${id}`
                };
                userNotifs.unshift(newNotif);
                changed = true;

                // Also show a real-time toast if on any page
                if (typeof showToast === 'function') {
                    showToast(`🚀 خبر عاجل: صدرت لعبة ${game.name} الآن!`, 'success');
                }
            }
        }
    });
    ...
    if (changed) {
        allNotifs[userName] = userNotifs;
        localStorage.setItem('gv_notifications', JSON.stringify(allNotifs));
        
        // Update unread count badge globally if function exists
        if (typeof updateNavNotifBadge === 'function') {
            updateNavNotifBadge();
        }
    }
}
